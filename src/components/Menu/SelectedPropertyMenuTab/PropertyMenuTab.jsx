...
export default function PropertyMenuTab({
  PropertyTabItemsArray,
  totalUnits,
  activeUnits,
  propertyImage1,
  propertyImage2,
  ...
  return (
    <Box sx={{ width: "100%" }}>
      <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
        ...
        >
          ...
          {PropertyTabItemsArray.map((data, key) => {
            return (
              <Tab
                label={data}
                sx={{ width: "5vw", marginRight: "2vw" }}
                {...a11yProps(key)}
              />
            );
          })}
   ...
   At line 98 where data is ==="OVerview", we are trying to make sure that the totalUnits, activeUnits , propertyImage1 and propertyImage2 are specifically sent to the Overview Component ..  This will be done for passing specific values to the other components as well with the tenary operators */}
      ...
      {PropertyTabItemsArray.map((data, index) => {
        let TaskBarComponent = lazy(() =>
          import(`../../PropertyScreenComponent/Property${data}`)
        );
